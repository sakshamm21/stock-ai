...
function PureMultimodalInput({
  ...
  return (
    <div className="relative w-full flex flex-col gap-4">
      ...
      <Textarea
        ref={textareaRef}
        placeholder="Ask about stocks, financials, or market news... type @ for tickers"
        value={input}
        onChange={handleInput}
        onKeyDown={handleKeyDown}
        className={cx(
          'min-h-[24px] max-h-[calc(75dvh)] overflow-hidden resize-none rounded-2xl !text-base bg-muted pb-10 dark:border-zinc-700 focus-visible:ring-primary/30',
          className,
        )}
        rows={2}
        autoFocus
...
function PureSendButton({
  ...
  return (
    <Button
      className="rounded-full p-1.5 h-fit bg-primary text-primary-foreground hover:bg-primary/90 border border-primary/20 shadow-sm"
      onClick={(event) => {
        event.preventDefault();
        submitForm();
      }}
