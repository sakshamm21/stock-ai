...
export async function POST(request: Request) {
  ...
  // Fall back to environment variables when keys are not provided by the client
  const resolvedModelApiKey = modelApiKey || process.env.OPENAI_API_KEY;
  const resolvedFinancialDataApiKey =
    financialDataApiKey || process.env.FMP_API_KEY;

  if (!resolvedModelApiKey) {
    return new Response('Model API key is required', { status: 400 });
  }

  if (!resolvedFinancialDataApiKey) {
    return new Response('Financial Data API key is required', { status: 400 });
  }
  ...
  return createDataStreamResponse({
    execute: async (dataStream) => {
      ...
      const financialToolsManager = new FinancialToolsManager({
        financialDataApiKey: resolvedFinancialDataApiKey,
        dataStream,
      });
